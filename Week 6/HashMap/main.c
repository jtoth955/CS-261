 ...
 * Name: John R Toth
 * Date: 8/04/15
 * Solution description: main hashMap implementation file.
 */
...
int main (int argc, const char * argv[]) {
    ...
    fileptr = fopen(filename, "r");             //open input1.txt as read only
    
    char *word = getWord(fileptr);
        while(1)                                //intentional continuous loop until broken
        {
            word = getWord(fileptr);            //word gets the current word from the file
            if(word == NULL)
                break;
            
            if(atMap(hashTable, word) == NULL)
            {
                insertMap(hashTable, word, 1);      //creates a new entry
            }
