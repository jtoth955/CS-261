 ...
 * Name: John R Toth
 * Date: 8/10/15
 * Solution description: graph implementation file.
 */
...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    addFrontCirListDeque(stack, temp);              //add to front of stack
    
    while(!isEmptyCirListDeque(stack))
    {
        temp = frontCirListDeque(stack);            //temp get's value at front and
        removeFrontCirListDeque(stack);             //removes it from front (DFS)
        
        if(!temp->isVisited)                        //temp gets 1 once visited
        {
            temp->isVisited = 1;
        }
        if(temp == destination)                     //if temp equals destination, return true
        {
            return 1;
        }
        
        for(int i = 0; i < temp->numNeighbors; i++) //check neighbors, if not visited, add to stack
        {
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    {
        temp = frontCirListDeque(stack);            //temp get's value at front and
        removeBackCirListDeque(stack);              //removes it from the back (BFS)
