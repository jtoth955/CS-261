 ...
 * Name: John R Toth
 * Date: 7/18/15
 * Solution description: Binary Search Tree implementation file.
 */
...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    if(compare(val, cur->val) <= 0)            //compare if value is "<=" current val
    {
        cur->left = _addNode(cur->left, val);   //set left Node to val if so
    }
    
    else
    {
        cur->right =_addNode(cur->right, val);  //set right Node to val otherwise
    }
    
    
    return cur;                                 //return cur, recursive loop continues
}
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    struct Node *current = tree->root;              //current points to tree root
    
    while(current != 0) {
        
        if(compare(val, current->val) == 0)         //if equal, return "true"
        {  
            return 1;
        }
        
        else if(compare(val, current->val) == -1)   //if val < current val
        {
            current = current->left;                //current now equals left node
        }
...
assert(cur != 0);
    ...
    if(cur->left == NULL)                           //if no left node exists, then this is a "special case", as discussed
        //in the lecture
    {
        return cur->right;                          //right node is then returned
    }
    
    else
    {
        cur->left = _removeLeftMost(cur->left);     //otherwise, there should be a left node and it will be passed to the
                                                    //removeLeftMost fn.
    }
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
    ...
    if(compare(val, cur->val) == 0)                     //base case, found!
    {
        if(cur->right == NULL)                             //if right is null, return left node
            return cur->left;
        
        else
        {
            cur->val = _leftMost(cur->right);           //current val is replaced with leftmost child of right child
            cur->right = _removeLeftMost(cur->right);   //remove leftmost node, set right child
        }
    }
    
    else if(compare(val, cur->val) == -1)               //recursive call
    {
        cur->left = _removeNode(cur->left, val);        //if val "<" current, current == left
    }
    
    else
    {
        cur->right = _removeNode(cur->right, val);      //if val ">" current, current == right
    }
